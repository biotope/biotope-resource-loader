(function (global, factory) {
  ...
  const update = (container, force = false) => {
    // ☕ Merging all
    ...
    for (const pkgKey in queue.componentResources) {
      const pkg = queue.componentResources[pkgKey];
      for (const request of queue.requests) {
        if (request.fetch === 'done') {
          for (const pathKey in pkg.paths) {
            const path = pkg.paths[pathKey];
            if (path === request.path) {
              queue.readyComponentResources = produce(
                  queue.readyComponentResources,
                  draftState => {
                    draftState[pkgKey].paths.splice(pathKey, 1);
                  }
              );
            }
          }
          for (const dependsOnKey in pkg.dependsOn) {
            const path = pkg.dependsOn[dependsOnKey];
            if (path === request.path) {
              queue.readyComponentResources = produce(
                  queue.readyComponentResources,
                  draftState => {
                    draftState[pkgKey].dependsOn.splice(dependsOnKey, 1);
                  }
              );
            }
          }
      ...
      } else if (updatedPkg.dependsOn && updatedPkg.dependsOn.length === 0) {
        getPackages(addToQueue([updatedPkg]), true);
      }
    }
    getPackages(queue.requests);
